...
- Policy can directly attach to user, called an Inline Policy.
...
### Policy:
- Managed Pol.: managed by aws, can not edit, have orange box icon
- Customer Pol.
- Inline Pol.
  ![](Policy_1.png)
...
### S3 Key Details:
- Objects (regular files or directories) are stored in S3 with:	
	+ key
	+ value
	+ version ID
  + metadata
...
- For security, data transfer must complete within 90 days.
- Can Inport/Export from s3.
- Can Inport from S3 or Glacier.
- 2 type: 50 TB and 80 TB.
...
- **On-Demand instances** (default) are based on a fixed rate by the hour or second. As the name implies, you can start an On-Demand instance whenever you need one and can stop it when you no longer need it. There is no requirement for a long-term commitment.
...
- **Spot instances** take advantage of Amazon’s excess capacity and work in an interesting manner. In order to use them, you must financially bid for access. Because Spot instances are only available when Amazon has excess capacity, this option makes sense only if your app has flexible start and end times. You won’t be charged if your instance stops due to a price change (e.g., someone else just bid a higher price for the access) and so consequently your workload doesn’t complete. However, if you terminate the instance yourself you will be charged for any hour the instance ran. Use case: normally used in batch processing jobs (non-critical bg job); can handle interrupt.
   
- **Dedicated Host**: use seperate hardware. Multi tenant (like apartment) and Single tenant (like house). Use when have strict server-bound licensing.
...
- Reserved Ins can shared between multi account within org.
...
### AMI (Amz Machine Image):
- Hold following info:
  + A template for root volume (EBS Snapshot or Instance store template) such as OS, Application server...,
  + Launch permission (which account can use AMI),
  + A block device mapping, specifies the volume to attach.
- AMI is Region specific.
  
### Amazon Marketplace:
- Where you can purchasing AMI.
...
- Instance with Instance Store-backed: can not be stop, only terminated. Data will be lost if health host fail or instance terminated.
...
- CloudTrail can be set to delivery events to CloudWatch.
...
- While EBS mounts one EBS volume to one instance, you can attach one EFS volume across multiple EC2 instances (same VPC).ddd
...
- It is spread across 3 geographically distinct data centers (3 regions).
...
### Redshift Simplified:
 ...
 Also analyze (run complex SQL) on massive amount data.
...
-  Redshift can be single node with 160GB, multi node up to 128 compute node.
...
- ElasticCache is only accessible from same VPC
...
- **Latency-based Routing**, Control route base on which lowest latency (usually server nearest to user). To use latency-based routing, you must create a latency resource record set in the same region as the corresponding EC2 or ELB resource receiving the traffic. When Route53 receives a query for your site, it selects the record set that gives the user the quickest speed. When creating Latency-based Routing, you need to specify a new record for each IP.
- **Failover Routing** is used when you want to configure an active-passive failover set up. Set which Primary, which Secondary. Route53 auto monitor the health of your primary so that it can failover when needed (switch to Secondary). You can also manually set up health checks to monitor all endpoints if you want more detailed rules.
...
### Health check:
- Every 30s by default, can reduce 10s.
- Can monitor other health check.

### Resolver:
- Known as .2 resolver,
- Route DNS query between your VPC and your network (on-premise),
- Regional service, hybrid on-premise and cloud,
...
### ELB Key Details:
- Component:
  + Listeners: receive traffic, match with port, determine what to do, if ok, send traffic to Target Group (EC2 inside).
  + Rules: only for ALB, determine which path to forward.
  + Target Group: EC2 registered.
...
- Auto scale can via:
  ...
  + Scaling Policy (ex: policy if exceed 75% CPU, add another server)
- Launch configuration: same process as Launch EC2 except not launch EC2. It's can not edit.
...
- VPC is region specified and not span region.
...
- VPCs are region specific and you can have up to 5 VPCs per region. 200 subnet per VPC.
...
### Security Group
- An instance can belong many SG, only 1 SG have Allow an IP, that IP will be allowed.
- Limit: 60 inbound, 60 outbound rule.
...
- Create NAT:
   + Create NAT with public subnet
   + Update Route table for route from private EC2 to this NAT.
...
- Create IG:
   + Create IG,
   + Attach TG to VPC
   + Update Route table for route from VPC to this IG.
...
### VPC Endpoints:
- Private connect from VPC to other Aws service (within aws network), like EC2 call S3 (without via Internet as usual).
- Instance in VPC not required IP public.
...
- **Gateway Endpoints** rely on creating entries in a route table and pointing them to private endpoints used for S3 or DynamoDB. Gateway Endpoints are mainly just a target that you set. Free.
...
- You can create a VPC peering connection between your own VPCs, across regions or different AWS account.
...
- It is worth knowing what VPC peering configurations are not supported:
  - Overlapping CIDR Blocks (172.0.0.0/16 can not peer 172.0.0.0/16)
  - Transitive Peering (VPC A want to talk VPC B, must peering directly between A, B, not through C,D...)
...
- Message size: can be 1 byte to 256 KB.
...
- There are three different types of Kinesis:
  ...
  - Kinesis Firehose
    ...
    - You choose **1 consumer**, data **immediately disappear once it's consumed**.
  ...
  - Kinesis Analytics
    - Kinesis Analytics works with both Kinesis Streams and Kinesis Firehose and can analyze data on the fly. You can use SQL and return result to output.
...
- AWS has servers preconfigured (just sitting around turned off) for your runtime environment. When a Lambda is invoked these servers need to be turned on and your code needs to be copied over. During the time there will be a delay when the function will initially run which is called a Cold Start.
